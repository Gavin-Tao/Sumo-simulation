...
class TrafficSignal:
    ...
    def __init__(
        ...
        self.ctb_alpha = ctb_alpha
        self.ctb_beta  = ctb_beta
        self.ctb_gamma = ctb_gamma
    ...
    def get_CTB_priority_pressure(self) -> float:
        """
        ...
        for sign, lanes in ((1, self.out_lanes), (-1, self.lanes)):
            for lane in lanes:
                for vid in get_ids(lane):
                    vtype = get_type(vid)
                    if vtype == "truck":
                        pressure += self.ctb_beta * sign
                    elif vtype == "bus":
                        pressure += self.ctb_gamma * sign
                    else:
                        pressure += self.ctb_alpha * sign
