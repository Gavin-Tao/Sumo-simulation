...
class SumoEnvironment(gym.Env):
    ...
    def record_vehicle_metrics(self):
        """记录每个车辆的metrics而不是按step"""
        ...
        for vid in current_vehicles:
            if vid not in self.vehicle_metrics:
                self.vehicle_metrics[vid] = {
                    ...
                    'route_id': self.sumo.vehicle.getRouteID(vid),
                    'waiting_times': [],
                    'speeds': [],
                    'positions': [],
                    'lanes': []
        ...
        for vid in current_vehicles:
            ...
            metrics['waiting_times'].append(self.sumo.vehicle.getWaitingTime(vid))
            metrics['speeds'].append(self.sumo.vehicle.getSpeed(vid))
            metrics['positions'].append(self.sumo.vehicle.getPosition(vid))
            metrics['lanes'].append(self.sumo.vehicle.getLaneID(vid))
            metrics['last_update'] = self.sim_step
        
        # 处理已经消失的车辆（完成生命周期）
        previous_vehicles = set(self.vehicle_metrics.keys())
        departed_vehicles = previous_vehicles - current_vehicles
        
        for vid in departed_vehicles:
            if 'exit_time' not in self.vehicle_metrics[vid]:
                self.vehicle_metrics[vid]['exit_time'] = self.sim_step
                self.vehicle_metrics[vid]['total_waiting_time'] = sum(self.vehicle_metrics[vid]['waiting_times'])
                self.vehicle_metrics[vid]['avg_speed'] = np.mean(self.vehicle_metrics[vid]['speeds'])
                # 移到完成的车辆记录中
                self.vehicle_lifecycle[vid] = self.vehicle_metrics[vid]

    def _sumo_step(self):
        if self.evaluation:
            # self.record_vehicle_metrics()  # 替换原来的evaluation_metrics_CTB()
