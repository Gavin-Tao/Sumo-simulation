"""FixedCoeffDQN training script (Shi et al. TPAMI 2024 reward baseline).

Cooperative reward formula:
  eff_r = α · own_r + (1-α) · mean(active neighbor rewards)
  α = 0.5 fixed (TPAMI paper grid-search optimum)

Differences from traincoeff_gat.py:
  - Agent: FixedCoeffDQN (no GAT, uniform mean over active neighbors)
  - No learnable cooperation params — only Q-net optimized
  - Wandb logs reward magnitude debug metrics (no β to log)

Usage:
    python experiments/traincoeff_tpami.py --config experiments/configs/expXX_tpami.yaml
"""
...
from sumo_rl.agents.coefficient import FixedCoeffDQN
...
def save_checkpoint(agent: FixedCoeffDQN, episode: int, model_dir: str, filename = None) -> str:
    os.makedirs(model_dir, exist_ok=True)
    checkpoint = {
        "policy_state_dict":    agent.q_net.state_dict(),
        "target_state_dict":    agent.target_q_net.state_dict(),
        "optimizer_state_dict": agent.optimizer.state_dict(),
        # FixedCoeffDQN: no learnable cooperation module — only Q-net + α scalar.
        "alpha":                agent.alpha,
        "episode":              episode,
    }
...
def train(cfg: dict, timestamp: str):
    ...
    for _ in range(1, cfg.get("runs", 1) + 1):
        ...
        agent = FixedCoeffDQN(
            ...
            device=device,
            # FixedCoeffDQN-specific (just α — no learnable cooperation module)
            ...
            if agent.start_train and logging_mode != "none":
                ...
                if attn is not None:
                    print(f"[{exp_name}] ep={episode:5d}  epsilon={agent.epsilon:.4f}  "
                          f"α={attn['alpha']:.2f}  n_active={attn['n_active_avg']:.2f}  "
                          f"own_r={attn['own_reward_mean']:.2f}  nb_mean={attn['nb_reward_mean']:.2f}  "
                          f"eff_r={attn['eff_reward_mean']:.2f}  "
                          f"|nb|/|own|={attn['nb_to_own_abs_ratio']:.2f}")
                else:
                    print(f"[{exp_name}] ep={episode:5d}  epsilon={agent.epsilon:.4f}  α={agent.alpha:.2f}  (no update yet)")
                ...
                if logging_mode == "simple":
                    ...
                    if attn is not None:
                        # FixedCoeffDQN: no GAT-β, just log reward magnitudes + alpha + n_active
                        for k in ("alpha", "n_active_avg",
                                  "nb_reward_sum_raw", "nb_reward_mean",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FixedCoeffDQN: TPAMI-style fixed α=0.5 + uniform-mean neighbor reward baseline")
