...
#设置SummaryWriter的路径
WRITER_PATH = "./logs/1x1/4_phases/" + TIMESTAMP
...
if __name__ == "__main__":
    ...
    out_csv = f"outputs/syc/1x1/4phases/{experiment_time}_alpha{args.alpha}_gamma{args.gamma}_eps{args.epsilon}_decay{args.decay}"
        ...
        for episode in range(1, episodes + 1):
            ...
            if dqn_agents[env.ts_ids[0]].start_train:
                if episode != 0 and episode % checkpoint_interval == 0:
                    ...
                    if os.path.exists("./models/1x1/4phases/checkpoint") is False:
                        os.makedirs("./models/1x1/4phases/checkpoint")
                    ckpt_timestamp = "{0:%Y-%m-%dT%H-%M-%S}".format(datetime.now())
                    path_checkpoint = "./models/1x1/4phases/checkpoint/ckpt_{0}_{1}.pth".format(ckpt_timestamp, episode)
