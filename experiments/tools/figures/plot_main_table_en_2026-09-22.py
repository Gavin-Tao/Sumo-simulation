...
每行更优者标绿; 保序门: 严格 amb ≤ bus ≤ car 通过写 pass; 严格不过但相邻类差 ≤ 5 s (一个决策步 / 5 s 采样量子)
写 pass (tol. 5 s); 否则 fail。数值来自 DUBLIN_02H_18H_VERDICT_2026-09-22.txt / 1X1 / 1X3 / DUBLIN_208_VS_211 文档。
...
ROWS=[("car","Stopped time / visit (s) - car"),("bus","Stopped time / visit (s) - bus"),("amb","Stopped time / visit (s) - ambulance"),("all","Stopped time / visit (s) - all"),("stops","Stop events / visit - all"),("J","J(531) weighted cost (per visit)")]
...
for scen,d,wins in S:
    ...
    cells.append(["Ordering gate: amb ≤ bus ≤ car",g[0],g[1],"",""]); colors.append(["#ffffff"]*5)
fig,ax=plt.subplots(figsize=(13.5,22.5)); ax.axis("off")
