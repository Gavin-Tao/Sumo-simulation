...
if __name__ == "__main__":
    ...
    prs.add_argument(
        ...
        dest="route",
        type=str,
        default="nets/syc/intersection_heavy_new.rou.xml",
    ...
    env = SumoEnvironment(
        net_file="nets/syc/intersection_new.net.xml",
    ...
    for run in range(1, args.runs + 1):
        ...
        env.close()
"""       
        if FLAGS.mode == 'train' and agent.start_train:
            if episode != 0 and episode % checkpoint_interval == 0:
                ...
                if os.path.exists("./models/checkpoint") is False:
                    os.makedirs("./models/checkpoint")
                ckpt_timestamp = "{0:%Y-%m-%dT%H-%M-%S}".format(datetime.now())
                path_checkpoint = "./models/checkpoint/ckpt_{0}_{1}.pth".format(ckpt_timestamp, episode)
