"""Original CoLight DQN training script (Wei et al., KDD 2019).

Differences from traincolight.py:
  - Uses CoLightOrigDQN: multi-head GAT with LeakyReLU attention and ELU output.
  - GAT follows Veličković et al. (ICLR 2018):
      e_ij = LeakyReLU(a^T [W·h_i || W·h_j])
      α_ij = softmax_j(e_ij)
      h'_i = ELU(Σ α_ij · W·h_j)
  - Extra config parameter: n_heads (default 2).
...
Usage:
    python experiments/trainorico.py --config experiments/configs/exp87_colightorig_1x3.yaml
"""
...
def train(cfg: dict, timestamp: str):
    ...
    if "obs_downstream" in cfg:      # ψ downstream block (E1b)
    ...
    if "obs_pad_tokens" in cfg:      # T heterogeneous nets
            ...
            try:
                while not done["__all__"]:
                    ...
                    if agent.replay_buffer.size() > agent.mini_size:
                        ...
                        if agent.use_per:
                            b_s, b_a, b_r, b_ns, b_d, b_nb, b_next_nb, b_w, b_idx = \
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Original CoLight DQN trainer")
