        ...
        for episode in range(1, episodes + 1):
            ...
            if dqn_agent.start_train:
                if episode != 0 and episode % checkpoint_interval == 0:
                    ...
                    if os.path.exists("./models/3x3/txw_4phases/checkpoint") is False:
                        os.makedirs("./models/3x3/txw_4phases/checkpoint")
                    ckpt_timestamp = "{0:%Y-%m-%dT%H-%M-%S}".format(datetime.now())
                    path_checkpoint = "./models/3x3/txw_4phases/checkpoint/ckpt_{0}_{1}.pth".format(ckpt_timestamp, episode)
